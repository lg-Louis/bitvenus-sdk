...
//        request.setListenKey(listenKey);
        try {
            ...
            String channel = MAPPER.writeValueAsString(request);
            return createNewUserWebSocket(channel, listenKey, listener, retry);
        } catch (JsonProcessingException e) {
    ...
    private Closeable createNewUserWebSocket(String channel, String listenKey, BrokerApiWebSocketUserListener<?> listener, boolean retry) {
        val url = wsApiUserUrl + listenKey;
        Request request = new Request.Builder().url(url).build();
        listener.setFailure(false);
        final WebSocket webSocket = client.newWebSocket(request, listener);
        log.info(" connect to {} \n and send > {}", url, channel);
//        webSocket.send(channel);
        Closeable closeable = createCloseable(listener, webSocket);
        if (retry) {
            // 添加监控线程
            ...
            while (true) {
                try {
                    ...
                    if (listener.getFailure()) {
                        log.info("user socket monitor : retry to connect");
                        createNewUserWebSocket(channel, listenKey, listener, false);
                    }
                } catch (Exception e) {
